...
| `<UsageExplorer>` | DS | DesignSystem | The one "where is this used?" surface. Pick a token / component / page → full bidirectional context (token→components+pages · component→tokens+pages+nesting · page→components+tokens). Chip behavior follows the data: a component/page **with a story** is a chip whose **whole pill** opens that story (click anywhere — marker, label, or ↗); a **token** or a not-yet-documented entity instead **walks the graph in place**. Graph-walking on storied entities stays available from the left list. The token tab carries an in-view **lane filter** (color · typography · scale …, derived from the data) so the whole type system or palette reads in one place — including Tailwind-default type utilities (`text-sm`, `font-medium`) surfaced from real className usage. Reads `component-pages.json` (`tokens[]` w/ `category` + `components[].tokens` + `fileIndex`). TokenMatrix's capped chips deep-link in via a `usageExplorerStoryId` + `&args=focus:<token>`. |
| `<IconMatrix>` | DS | DesignSystem | Live iconography coverage: which icons the app imports, which it renders (and how often), at what pixel sizes (Tailwind `h-*`/`size-*` → px), **and where** — each icon resolves to the precise components & pages it's used in (with the render-site count per component, each chip clicking into its story). A size histogram + per-icon × size grid + a "where each icon is used" section + "imported but not rendered". Scans `/src/**` raw at build time (`import.meta.glob`) and resolves render-site files through the shared usage graph (`component-pages.json` fileIndex via `resolveUsage`), degrading to file names when that graph isn't generated. **No icon-pkg install**; library-agnostic — the story passes `library` metadata + a `resolve(name)→component` fn (lucide-react / phosphor / heroicons / …). |
