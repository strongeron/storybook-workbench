...
// Bare-channel tokens (withOklch / shadcn-HSL) hold raw channels like `0.95 0.001 234`
// or alias to one via `var(--x)`. Painting `background: var(--token)` reads transparent
// because a bare triplet is not a valid CSS color unwrapped. resolvePaint probes the live
// element and retries oklch()/hsl(), leaving the working wrapper painted on the swatch.
function useResolvedPaint(name: string) {
  const ref = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (ref.current) resolvePaint(ref.current, name);
  }, [name]);
  return ref;
}

function ColorSwatch({ token }: { token: Token }): JSX.Element {
  const swatchRef = useResolvedPaint(token.name);
  return (
    <div style={CARD}>
      <div ref={swatchRef} style={{ ...SWATCH, background: token.value }} />
